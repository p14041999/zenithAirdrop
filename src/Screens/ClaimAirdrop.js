import React, { Component } from 'react'
import {toast} from 'react-toastify';
import Web3 from "web3";
import Web3Modal from "web3modal";
import WalletConnectProvider from '@walletconnect/web3-provider';
import {abi} from '../abi.json';
const ContractAddress = "0x44526888F574Aeb033626874677625A8252fd38b"
const providerOptions = {
  // Example with injected providers
  injected: {
    display: {
      logo: "data:image/svg+xml;base64,",
      name: "Metamask",
      description: "Connect with the provider in your Browser"
    },
    package: null
  },
  // Example with WalletConnect provider
  walletconnect: {
    display: {
      logo: "data:image/svg+xml;base64,",
      name: "Wallet Connect",
      description: "Scan qrcode with your mobile wallet"
    },
    package: WalletConnectProvider,
    options: {
        rpc: {
            56: "https://bsc-dataseed.binance.org/"
        }
    }
  }
};
const web3Modal = new Web3Modal({providerOptions});

export default class ClaimAirdrop extends Component {
    state ={
        isConnected:false,
        provider:null,
        amount:'0',
        mail:''
    }
    async componentDidMount(){
        await this.connect();
        if(this.state.provider !== null){
            let web3 = new Web3(this.state.provider);
            let contract = new web3.eth.Contract(abi,ContractAddress);
            let min_amount = await contract.methods.getMinAmount().call();
            // console.log(this.state.provider);
            min_amount = Number.parseInt(min_amount)+1;
            this.setState({amount:min_amount.toString()});
        }
    }
    connect = async()=>{
        let provider = await web3Modal.connect();
        this.setState({provider,isConnected:true});
        // Subscribe to accounts change
            provider.on("accountsChanged", (accounts) => {
                toast.info("Accounts Updated!", {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
                    pauseOnHover: true,
                    draggable: true,
                    progress: undefined,
                });
            });
            
            // Subscribe to chainId change
            provider.on("chainChanged", (chainId) => {
                if(chainId != 56){
                    toast.warn("Please Switch to BSC!", {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: false,
                        closeOnClick: true,
                        pauseOnHover: true,
                        draggable: true,
                        progress: undefined,
                    });
                }
                console.log(chainId);
            });
            
            // Subscribe to provider connection
            provider.on("connect", (info) => {
                if(info.chainId != 56){
                    toast.warn("Please Switch to BSC!", {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: false,
                        closeOnClick: true,
                        pauseOnHover: true,
                        draggable: true,
                        progress: undefined,
                    });
                }
                console.log(info);
            });
            
            // Subscribe to provider disconnection
            provider.on("disconnect", (error) => {
                // toast.error(error.message, {
                //     position: "top-right",
                //     autoClose: 5000,
                //     hideProgressBar: false,
                //     closeOnClick: true,
                //     pauseOnHover: true,
                //     draggable: true,
                //     progress: undefined,
                // });
                console.log(error.message);
            });
        return provider;
    }
    claimAirDrop= async (e)=>{
        e.preventDefault();
        try{
        if(this.state.provider === null){
            await this.connect();
        }
        let web3 = new Web3(this.state.provider);
        let accounts = await web3.eth.getAccounts();
        // console.log(accounts);
        let contract = new web3.eth.Contract(abi,ContractAddress);
        // let gas = await contract.methods.airdrop(this.state.mail).estimateGas({from:accounts[0],value:this.state.amount});
        // console.log(gas);
        contract.methods.airdrop(this.state.mail).send({from:accounts[0],value:this.state.amount,gas:120858})
        .on('transationHash',(hash)=>{
            console.log(hash);
            toast.info("Txn Created"+hash.substr(0,5)+'...'+hash.substr(hash.length-3,3), {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
                pauseOnHover: true,
                draggable: true,
                progress: undefined,
            });
        }).once('confirmation',(number,receipt)=>{
            let hash = receipt.transactionHash;
            if(receipt.status){
                toast.success("Successfully created transacrtion "+hash.substr(0,5)+'...'+hash.substr(hash.length-3,3), {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
                    pauseOnHover: true,
                    draggable: true,
                    progress: undefined,
                });
            }else{
                toast.error("Transaction Failed !", {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
                    pauseOnHover: true,
                    draggable: true,
                    progress: undefined,
                });
            }
        }).on('error', function(error, receipt) {
            console.log(error);
        });
    }catch(e){
        // // if(e)
        //     console.log(e.originalError);
    }
    }
    render() {
        return (
            <div className="container-fluid p-0">
                <nav class="navbar navbar-light bg-light">
                    <div class="container">
                        <a class="navbar-brand" href="#">
                        <img src="/logo.png" alt="" width="50" height="50" />
                        </a>
                    </div>
                </nav>
                <div className="container py-5 mt-4">
                    <div className="row d-flex justify-content-center">
                        <div className="col-12 col-md-6 col-lg-4">
                            <div className="card margin-custom shadow border-0">
                                <div className="card-body">
                                    <div className="px-3">
                                        <div className="mb-5 d-flex justify-content-center">
                                            <img src="/logo.png" className="img-fluid" width="80" alt="" />
                                        </div>
                                        <div className="my-5 d-flex justify-content-center">
                                            <h4>Get Airdrop of 100 ZENITH</h4>
                                        </div>
                                        <div className="mt-5 d-flex justify-content-center pb-4">
                                            Join Our Social Pages
                                        </div>
                                        <div className="mb-5">
                                            <div className="d-flex justify-content-between align-items center">
                                                <a href="https://medium.com/@zenithchain" target="_blank"><img src="/medium.png" height="25"></img> </a>
                                                <a href="https://www.linkedin.com/company/zenith-chain/" target="_blank"><img src="/linkedin.png" height="25"></img></a>
                                                <a href="https://twitter.com/zenithchain_co" target="_blank"><img src="/twitter.png" height="25"></img></a>
                                                <a href="https://t.me/joinchat/p124xPC3l083NjAx" target="_blank"><img src="/telegram.png" height="25"></img></a>
                                            </div>
                                        </div>
                                        <form onSubmit={this.claimAirDrop}>
                                            <div className="mt-5 pb-4">
                                            <div class="form-floating mb-3">
                                                <input type="email" 
                                                class="form-control" 
                                                value={this.state.mail}
                                                onChange={e=>{
                                                    this.setState({mail:e.target.value});
                                                }}
                                                id="floatingInput" placeholder="name@example.com"/>
                                                <label for="floatingInput">Email address</label>
                                            </div>
                                            </div>
                                            <div className="mb-3 d-flex justify-content-center">
                                                {this.state.isConnected && <button className="btn btn-lg btn-primary w-100 shadow" type="submit"> Claim Airdrop Now</button> }
                                                {!this.state.isConnected && <button className="btn btn-lg btn-primary w-100 shadow" onClick={this.connect} type="button"> Connect Wallet</button> }
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
            </div>
        )
    }
}
